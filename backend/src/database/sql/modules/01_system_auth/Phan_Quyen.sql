...
/* ==================================================
   1. TẠO ROLE SV NẾU CHƯA CÓ
   ================================================== */

IF NOT EXISTS (
    SELECT 1 
    FROM sys.database_principals 
    WHERE name = 'SV' AND type = 'R'
)
BEGIN
    CREATE ROLE SV;
END
...
/* ==================================================
   2. THÊM USER login_sv VÀO ROLE SV NẾU CHƯA THÊM
   ================================================== */

IF DATABASE_PRINCIPAL_ID('login_sv') IS NOT NULL
BEGIN
...
-- SQL Server không cho GRANT INSERT theo từng cột
-- nên phải cấp INSERT ở mức bảng
GRANT INSERT ON dbo.DANGKY TO SV;
GO

-- SV chỉ được cập nhật cột HUYDANGKY
GRANT UPDATE ON dbo.DANGKY
(
    HUYDANGKY
)
...
-- SV không được xóa dòng đăng ký
